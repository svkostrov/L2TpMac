...
/usr/bin/grep -Fq '.foregroundStyle(stopButtonDisabled ? Color.secondary : Color.red)' L2TPOfficeApp/main.swift || fail "window destructive button text must use one red shade"
...
/usr/bin/grep -Fq '.frame(width: 360)' L2TPOfficeApp/main.swift || fail "menu bar popup width must stay compact"
/usr/bin/grep -Fq 'Text(appVersion)' L2TPOfficeApp/main.swift || fail "menu bar must show app version"
/usr/bin/grep -Fq '.font(.caption.weight(.medium))' L2TPOfficeApp/main.swift || fail "menu bar version text must stay compact"
